...
const qualEstado = function(siglaEstado){
   let estadoCidades = [];
   let erro = true;
   let se = siglaEstado
   if(siglaEstado != ""){
      ...
      cidades.forEach(item => {
         //Localiza a sigla do estado dentro o arra de cidades
      if(item.sigla.indexOf(se.toUpperCase()) == 0){
         //Percorre o array da chave de cidades dentro do estado que foi escontrado
         item.cidades.forEach(nomeCidades => {
            estadoCidades.push(nomeCidades.nome)
            erro = false;
         });
         }
      })
